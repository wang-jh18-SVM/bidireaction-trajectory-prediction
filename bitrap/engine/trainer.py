...
def do_train(cfg,
    ...
    with torch.set_grad_enabled(True):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            # optimize
            optimizer.zero_grad(
            )  # avoid gradient accumulate from loss.backward()
            loss.backward()
...
def do_val(cfg, epoch, model, dataloader, device, logger=None):
    ...
    with torch.set_grad_enabled(False):
        for iters, batch in enumerate(tqdm(dataloader), start=1):
            ...
            # compute loss
            loss = loss_dict['loss_goal'] + loss_dict['loss_traj'] + loss_dict[
                'loss_kld']
...
def inference(cfg,
    ...
    with torch.set_grad_enabled(False):
        ...
        if hasattr(all_distributions[0], 'mus'):
            distribution = model.GMM(
                torch.cat([d.input_log_pis for d in all_distributions],
                          axis=0),
        ...
        eval_results = evaluate_multimodal(all_pred_trajs,
                                           all_gt_trajs,
                                           mode=mode,
                                           distribution=distribution,
                                           bbox_type=cfg.DATASET.BBOX_TYPE)
        for key, value in eval_results.items():
            info = "Testing prediction {}:{}".format(
                key, str(np.around(value, decimals=3)))
            if hasattr(logger, 'log_values'):
                logger.info(info)
            else:
                print(info)
        ...
        if test_mode:
            ...
            output_file = os.path.join(
                cfg.OUT_DIR, '{}_{}.pkl'.format(cfg.MODEL.LATENT_DIST,
                                                cfg.DATASET.NAME))
    ...
    if eval_kde_nll:
        ...
        kde_nll_dataloader = DataLoader(dataloader.dataset,
                                        **dataloader_params)
        inference_kde_nll(cfg, epoch, model, kde_nll_dataloader, device,
                          logger)
...
def inference_kde_nll(cfg, epoch, model, dataloader, device, logger=None):
    ...
    with torch.set_grad_enabled(False):
        ...
        nll_dict = {
            'KDE_NLL': KDE_NLL
        } if cfg.MODEL.LATENT_DIST == 'categorical' else {
            'KDE_NLL': KDE_NLL,
            'Goal_NLL': Goal_NLL
        }
        info = "Testing prediction KDE_NLL:{:.4f}, per step NLL:{}".format(
            KDE_NLL, KDE_NLL_PER_STEP)
        if hasattr(logger, 'log_values'):
            logger.info(info)
        else:
            print(info)
