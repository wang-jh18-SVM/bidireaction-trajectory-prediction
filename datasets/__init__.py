...
from .HighD import HighDDataSet as HighD
...
def make_dataset(cfg, split):
    try:
        ...
        raise NameError(
            "Unknown method and dataset combination:{} + {}".format(
                cfg.METHOD, cfg.DATASET.NAME))
...
def collate_dict(batch):
    ...
    for key in all_keys:
        ...
        if elem[key] is None:
            ...
            neighbor_dict = {
                sub_key: [b[key][sub_key] for b in batch]
                for sub_key in elem[key]
            }
            collate_batch[key] = dill.dumps(
                neighbor_dict) if torch.utils.data.get_worker_info(
                ) else neighbor_dict
        else:
